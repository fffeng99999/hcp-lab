...
def parse_ibft_metrics(log_dir: Path) -> Dict[str, float]:
    ansi_pattern = re.compile(r"\x1b\[[0-9;]*m")
    ...
    for log_file in log_dir.glob("**/*.log"):
        for line in log_file.read_text(encoding="utf-8", errors="ignore").splitlines():
            clean_line = ansi_pattern.sub("", line)
            if "ibft_metrics" not in clean_line.lower():
                continue
            for part in clean_line.split():
                if "=" not in part:
                    continue
    ...
    pre_prepare = key_values["pre_prepare_ms"]
    prepare = key_values["prepare_ms"]
    commit = key_values["commit_ms"]
    round_changes = key_values["round_changes"]
    total_messages = key_values["total_messages"]
    comm_bytes = key_values["comm_bytes"]

    return {
        "ibft_block_avg_ms": avg(block_times),
        "ibft_block_p99_ms": percentile(block_times, 99),
        "ibft_pre_prepare_avg_ms": avg(pre_prepare),
        "ibft_prepare_avg_ms": avg(prepare),
        "ibft_commit_avg_ms": avg(commit),
        "ibft_round_changes_avg": avg(round_changes),
        "ibft_total_messages_avg": avg(total_messages),
        "ibft_comm_bytes_avg": avg(comm_bytes),
    }
...
def parse_hierarchical_lightweight_tpbft_metrics(log_dir: Path) -> Dict[str, List[float]]:
    metrics = {
        "pre_prepare_ms": [],
        "prepare_ms": [],
        "commit_ms": [],
    ...
    for log_file in log_dir.glob("**/*.log"):
        for line in log_file.read_text(encoding="utf-8", errors="ignore").splitlines():
            clean_line = ansi_pattern.sub("", line)
            if "hierarchical_lightweight_tpbft_metrics" not in clean_line:
                continue
            for part in clean_line.split():
                if "=" not in part:
                    continue
...
def parse_hierarchical_hotspot_tpbft_metrics(log_dir: Path) -> Dict[str, List[float]]:
    metrics = {
        "pre_prepare_ms": [],
        "prepare_ms": [],
        "commit_ms": [],
    ...
    for log_file in log_dir.glob("**/*.log"):
        for line in log_file.read_text(encoding="utf-8", errors="ignore").splitlines():
            clean_line = ansi_pattern.sub("", line)
            if "hierarchical_hotspot_tpbft_metrics" not in clean_line:
                continue
            for part in clean_line.split():
                if "=" not in part:
                    continue
...
def parse_hierarchical_tpbft_parallel_block_metrics(log_dir: Path) -> Dict[str, List[float]]:
    metrics = {
        "pre_prepare_ms": [],
        "prepare_ms": [],
        "commit_ms": [],
    ...
    for log_file in log_dir.glob("**/*.log"):
        for line in log_file.read_text(encoding="utf-8", errors="ignore").splitlines():
            clean_line = ansi_pattern.sub("", line)
            if "hierarchical_tpbft_parallel_block_metrics" not in clean_line:
                continue
            for part in clean_line.split():
                if "=" not in part:
                    continue
