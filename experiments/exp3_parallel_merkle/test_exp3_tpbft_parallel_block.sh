...
HCPD_BINARY="../tests/exp3_parallel_merkle/bin/hcpd"
CLI_BINARY="tests/exp3_parallel_merkle/bin/hcpd"
...
python3 experiments/exp3_parallel_merkle/run_exp3_tpbft_parallel_block.py \
  ...
  --nodes "$NODE_COUNT" \
  --out "$REPORT_OUT" \
  --loadgen-args "--protocol grpc --grpc-endpoint http://127.0.0.1:$GRPC_PORT --rpc-endpoint tcp://127.0.0.1:$RPC_PORT --chain-id hcp-testnet-1 --keyring-backend test --keyring-home {data_root}/node1 --account-file {data_root}/accounts.jsonl --cli-binary $CLI_BINARY --send-amount 1 --fee-amount 1 --denom stake --account-count 100 --initial-nonce 0 --total-txs {tx} --target-tps 1000 --concurrency 32 --batch-size 1 --metrics-interval 100 --json-interval-ms 100 --csv-path $EXP_DIR/loadgen_k{k}_tx{tx}.csv"
