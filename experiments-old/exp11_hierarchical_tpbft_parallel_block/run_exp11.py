...
from collector.log_parser import parse_hierarchical_tpbft_parallel_block_metrics
...
def main() -> None:
    parser = argparse.ArgumentParser(description="实验十一：分层 TPBFT + 并行 Merkle 块")
    ...
    parser.add_argument("--out", type=str, default="experiments/exp11_hierarchical_tpbft_parallel_block/report")
    ...
    chain_id = os.environ.get("CHAIN_ID", "hcap-exp11")
    ...
    print(
        "实验十一参数: nodes={nodes} tx={tx} groups={groups} k={k} repeat={repeat} sig_algos={sig_algos} outer_mode={outer_mode} batch_verify={batch_verify} batch_size={batch_size}".format(
            nodes=args.nodes,
            tx=args.tx,
            groups=",".join(str(g) for g in groups),
            k=",".join(str(kv) for kv in k_list),
            repeat=args.repeat,
    ...
    aggregated: Dict[str, Dict[int, Dict[int, Dict[str, float]]]] = {}
    aggregated_std: Dict[str, Dict[int, Dict[int, Dict[str, float]]]] = {}
    ...
    for algo in sig_algos:
        ...
        for g in groups:
            ...
            s = args.nodes // g
            aggregated[algo][g] = {}
            aggregated_std[algo][g] = {}
            for k in k_list:
                samples: List[Dict[str, float]] = []
                for repeat_idx in range(1, args.repeat + 1):
                    data_root = artifact_root / f"algo_{algo}" / f"g_{g}" / f"k_{k}" / f"run_{repeat_idx}" / "data"
                    log_root = artifact_root / f"algo_{algo}" / f"g_{g}" / f"k_{k}" / f"run_{repeat_idx}" / "logs"
                    data_root.mkdir(parents=True, exist_ok=True)
                    log_root.mkdir(parents=True, exist_ok=True)

                    os.environ["CONSENSUS_ENGINE"] = "hierarchical-tpbft-parallel-block"
                    ...
                    print(
                        "实验点参数: algo={algo} g={g} s={s} k={k} run={run} data_root={data_root} log_root={log_root}".format(
                            algo=algo,
                            g=g,
                            s=s,
                            k=k,
                    ...
                    result = runner.run(
                        ...
                        description="验证分层 TPBFT 与并行 Merkle 块的协同性能",
                        matrix=matrix,
                        data_root=data_root,
                        log_root=log_root,
                    ...
                    exp_metrics = parse_hierarchical_tpbft_parallel_block_metrics(log_dir)
                    duration_s = float(metrics.get("duration_s", 0.0))
                    net_mbps = float(metrics.get("net_mbps", 0.0))
                    net_bytes_total = net_mbps * 1024.0 * 1024.0 / 8.0 * duration_s if duration_s > 0 else 0.0

                    sample = dict(metrics)
                    sample["pre_prepare_ms"] = avg(exp_metrics.get("pre_prepare_ms", []))
                    sample["prepare_ms"] = avg(exp_metrics.get("prepare_ms", []))
                    sample["commit_ms"] = avg(exp_metrics.get("commit_ms", []))
                    sample["comm_bytes_per_block"] = avg(exp_metrics.get("comm_bytes", []))
                    sample["total_messages"] = avg(exp_metrics.get("total_messages", []))
                    sample["sig_gen_count"] = avg(exp_metrics.get("sig_gen_count", []))
                    sample["sig_verify_count"] = avg(exp_metrics.get("sig_verify_count", []))
                    sample["sig_gen_time_ms"] = avg(exp_metrics.get("sig_gen_time_ms", []))
                    sample["sig_verify_time_ms"] = avg(exp_metrics.get("sig_verify_time_ms", []))
                    sample["aggregation_time_ms"] = avg(exp_metrics.get("aggregation_time_ms", []))
                    sample["verify_time_ms"] = avg(exp_metrics.get("verify_time_ms", []))
                    sample["sig_per_node"] = avg(exp_metrics.get("sig_per_node", []))
                    sample["sig_ops_per_tx"] = avg(exp_metrics.get("sig_ops_per_tx", []))
                    sample["batch_size"] = avg(exp_metrics.get("batch_size", []))
                    sample["batch_verify"] = avg(exp_metrics.get("batch_verify", []))
                    sample["verify_gain"] = avg(exp_metrics.get("verify_gain", []))
                    sample["sig_gen_parallelism"] = avg(exp_metrics.get("sig_gen_parallelism", []))
                    sample["sig_verify_parallelism"] = avg(exp_metrics.get("sig_verify_parallelism", []))
                    sample["sig_agg_parallelism"] = avg(exp_metrics.get("sig_agg_parallelism", []))
                    sample["sig_total_time_ms"] = (
                        sample["sig_gen_time_ms"]
                        + sample["sig_verify_time_ms"]
                        + sample["aggregation_time_ms"]
                    )
                    sample["merkle_block_time_ms"] = avg(exp_metrics.get("block_time_ms", []))
                    sample["merkle_subblock_time_ms"] = avg(exp_metrics.get("subblock_time_ms", []))
                    sample["merkle_merge_time_ms"] = avg(exp_metrics.get("merge_time_ms", []))
                ...
                aggregated[algo][g][k] = agg
                aggregated_std[algo][g][k] = agg_std
                points.append(
                    ExperimentPoint(
                        params={"algo": algo, "g": g, "s": s, "k": k, "nodes": args.nodes, "tx": args.tx},
                        metrics=agg,
                    )
                )
    ...
    result = ExperimentResult(
        name="实验十一：分层 TPBFT + 并行 Merkle 块",
        description="验证分层 TPBFT 通信复杂度优化与并行 Merkle 块计算的协同效果",
        points=points,
        metadata={
            "groups": groups,
            "k_list": k_list,
    ...
    for algo in sig_algos:
        for g in sorted(aggregated.get(algo, {}).keys()):
            s = args.nodes // g
            for k in sorted(aggregated[algo][g].keys()):
                metrics = aggregated[algo][g][k]
                stds = aggregated_std[algo][g][k]
                rows.append(
                    {
                        "algo": algo,
                        "g": g,
                        ...
                        "merkle_block_time_avg": metrics.get("merkle_block_time_ms", 0.0),
                        "merkle_subblock_time_avg": metrics.get("merkle_subblock_time_ms", 0.0),
                        "merkle_merge_time_avg": metrics.get("merkle_merge_time_ms", 0.0),
                    }
                )

    csv_path = output_dir / "exp11_summary.csv"
    ...
    # 计算加速比和效率（以每个 algo×g 组合下的 k=1 为基线）
    for algo in sig_algos:
        for g in sorted(aggregated.get(algo, {}).keys()):
            if 1 not in aggregated[algo][g]:
                continue
            base_merkle = aggregated[algo][g][1].get("merkle_block_time_ms", 0.0)
            for k in sorted(aggregated[algo][g].keys()):
                merkle_time = aggregated[algo][g][k].get("merkle_block_time_ms", 0.0)
                speedup = base_merkle / merkle_time if base_merkle > 0 and merkle_time > 0 else 0.0
                efficiency = speedup / k if k > 0 else 0.0
                aggregated[algo][g][k]["merkle_speedup"] = speedup
                aggregated[algo][g][k]["merkle_efficiency"] = efficiency
                for point in points:
                    if (point.params.get("algo") == algo
                            and point.params.get("g") == g
                            and point.params.get("k") == k):
                        point.metrics["merkle_speedup"] = speedup
                        point.metrics["merkle_efficiency"] = efficiency
    ...
    for algo in sig_algos:
        ...
        for k in k_list:
            ...
            valid_g = []
            for g in g_sorted:
                if k not in aggregated[algo][g]:
                    continue
                tps_values.append(aggregated[algo][g][k].get("tps", 0.0))
                valid_g.append(float(g))
            if tps_values:
                write_svg(
                    figures_dir / f"tps_vs_g_{algo}_k{k}.svg",
                    line_chart_svg(valid_g, tps_values, f"TPS vs g ({algo}, k={k})", "g", "TPS"),
                )
        ...
        for k in k_list:
            ...
            for g in g_sorted:
                if k not in aggregated[algo][g]:
                    continue
                sig_values.append(aggregated[algo][g][k].get("sig_total_time_ms", 0.0))
                valid_g.append(float(g))
            if sig_values:
                write_svg(
                    figures_dir / f"sig_time_vs_g_{algo}_k{k}.svg",
                    line_chart_svg(valid_g, sig_values, f"签名总耗时 vs g ({algo}, k={k})", "g", "time(ms)"),
                )

        # 按 g 分线的 Merkle 耗时 vs k
        for g in g_sorted:
            k_valid = []
            merkle_values = []
            for k in k_list:
                if k not in aggregated[algo][g]:
                    continue
                k_valid.append(float(k))
                merkle_values.append(aggregated[algo][g][k].get("merkle_block_time_ms", 0.0))
            if merkle_values:
                write_svg(
                    figures_dir / f"merkle_time_vs_k_{algo}_g{g}.svg",
                    line_chart_svg(k_valid, merkle_values, f"Merkle 耗时 vs k ({algo}, g={g})", "k", "time(ms)"),
                )
        ...
        for g in g_sorted:
            ...
            for k in k_list:
                if k not in aggregated[algo][g]:
                    continue
                k_valid.append(float(k))
                speedup_values.append(aggregated[algo][g][k].get("merkle_speedup", 0.0))
            if speedup_values:
                write_svg(
                    figures_dir / f"speedup_vs_k_{algo}_g{g}.svg",
                    line_chart_svg(k_valid, speedup_values, f"Merkle 加速比 vs k ({algo}, g={g})", "k", "Speedup"),
                )
        ...
        for g in g_sorted:
            ...
            for k in k_list:
                if k not in aggregated[algo][g]:
                    continue
                k_valid.append(float(k))
                eff_values.append(aggregated[algo][g][k].get("merkle_efficiency", 0.0))
            if eff_values:
                write_svg(
                    figures_dir / f"efficiency_vs_k_{algo}_g{g}.svg",
                    line_chart_svg(k_valid, eff_values, f"Merkle 效率 vs k ({algo}, g={g})", "k", "Efficiency"),
                )
    ...
    append_tps_vs_tx_by_nodes_chart(
        ...
        line_figure_name="exp11_tps_vs_tx_by_nodes.svg",
        bar_figure_name="exp11_tps_vs_tx_by_nodes_bar.svg",
        title="实验11 性能曲线（TPS）",
