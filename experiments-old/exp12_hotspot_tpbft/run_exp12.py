...
from collector.log_parser import parse_hierarchical_hotspot_tpbft_metrics
...
def main() -> None:
    parser = argparse.ArgumentParser(description="实验十二：热点账户感知分组 TPBFT")
    parser.add_argument("--groups", type=str, default="32,16,8,4,2")
    parser.add_argument("--strategies", type=str, default="random,hotspot")
    parser.add_argument("--zipf-alphas", type=str, default="0.0,1.5,1.8,2.0")
    parser.add_argument("--nodes", type=int, default=4)
    parser.add_argument("--tx", type=int, default=100)
    parser.add_argument("--repeat", type=int, default=5)
    parser.add_argument("--sig-algos", type=str, default="bls,ed25519")
    parser.add_argument("--out", type=str, default="experiments/exp12_hotspot_tpbft/report")
    ...
    parser.add_argument("--cross-group-penalty-factor", type=float, default=0.5)
    ...
    chain_id = os.environ.get("CHAIN_ID", "hcap-exp12")
    ...
    zipf_alphas = parse_float_list(args.zipf_alphas)
    ...
    print(
        "实验十二参数: nodes={nodes} tx={tx} groups={groups} strategies={strategies} zipf_alphas={zipf_alphas} repeat={repeat} sig_algos={sig_algos}".format(
            nodes=args.nodes,
            tx=args.tx,
            groups=",".join(str(g) for g in groups),
            strategies=",".join(strategies),
            zipf_alphas=",".join(str(a) for a in zipf_alphas),
            repeat=args.repeat,
            ...
            for strategy in strategies:
                for alpha in zipf_alphas:
                    ...
                    for repeat_idx in range(1, args.repeat + 1):
                        data_root = artifact_root / f"algo_{algo}" / f"g_{g}" / f"strategy_{strategy}" / f"alpha_{alpha}" / f"run_{repeat_idx}" / "data"
                        log_root = artifact_root / f"algo_{algo}" / f"g_{g}" / f"strategy_{strategy}" / f"alpha_{alpha}" / f"run_{repeat_idx}" / "logs"
                        data_root.mkdir(parents=True, exist_ok=True)
                        log_root.mkdir(parents=True, exist_ok=True)

                        os.environ["CONSENSUS_ENGINE"] = "hierarchical-hotspot-tpbft"
                        ...
                        os.environ["GROUPING_STRATEGY"] = strategy
                        os.environ["ZIPF_ALPHA"] = str(alpha)
                        if args.cross_group_penalty_factor > 0:
                            os.environ["CROSS_GROUP_PENALTY_FACTOR"] = str(args.cross_group_penalty_factor)
                        os.environ["INCLUDE_LOOPBACK"] = "true"

                        # loadgen 参数扩展
                        loadgen_selection = ""
                        if alpha > 0.0:
                            loadgen_selection = f"--account-selection-mode zipf --zipf-alpha {alpha}"
                        ...
                        print(
                            "实验点参数: algo={algo} g={g} s={s} strategy={strategy} alpha={alpha} run={run}".format(
                                algo=algo,
                                g=g,
                                s=s,
                                strategy=strategy,
                        ...
                        result = runner.run(
                            ...
                            description="热点负载下的分层 TPBFT 分组策略对比",
                            matrix=matrix,
                            data_root=data_root,
                            log_root=log_root,
                        ...
                        hotspot_metrics = parse_hierarchical_hotspot_tpbft_metrics(log_dir)
                        duration_s = float(metrics.get("duration_s", 0.0))
                        net_mbps = float(metrics.get("net_mbps", 0.0))
                        net_bytes_total = net_mbps * 1024.0 * 1024.0 / 8.0 * duration_s if duration_s > 0 else 0.0

                        sample = dict(metrics)
                        sample["pre_prepare_ms"] = avg(hotspot_metrics.get("pre_prepare_ms", []))
                        sample["prepare_ms"] = avg(hotspot_metrics.get("prepare_ms", []))
                        sample["commit_ms"] = avg(hotspot_metrics.get("commit_ms", []))
                        sample["comm_bytes_per_block"] = avg(hotspot_metrics.get("comm_bytes", []))
                        sample["total_messages"] = avg(hotspot_metrics.get("total_messages", []))
                        sample["sig_gen_count"] = avg(hotspot_metrics.get("sig_gen_count", []))
                        sample["sig_verify_count"] = avg(hotspot_metrics.get("sig_verify_count", []))
                        sample["sig_gen_time_ms"] = avg(hotspot_metrics.get("sig_gen_time_ms", []))
                        sample["sig_verify_time_ms"] = avg(hotspot_metrics.get("sig_verify_time_ms", []))
                        sample["aggregation_time_ms"] = avg(hotspot_metrics.get("aggregation_time_ms", []))
                        sample["verify_time_ms"] = avg(hotspot_metrics.get("verify_time_ms", []))
                        sample["sig_per_node"] = avg(hotspot_metrics.get("sig_per_node", []))
                        sample["sig_ops_per_tx"] = avg(hotspot_metrics.get("sig_ops_per_tx", []))
                        sample["batch_size"] = avg(hotspot_metrics.get("batch_size", []))
                        sample["batch_verify"] = avg(hotspot_metrics.get("batch_verify", []))
                        sample["verify_gain"] = avg(hotspot_metrics.get("verify_gain", []))
                        sample["sig_gen_parallelism"] = avg(hotspot_metrics.get("sig_gen_parallelism", []))
                        sample["sig_verify_parallelism"] = avg(hotspot_metrics.get("sig_verify_parallelism", []))
                        sample["sig_agg_parallelism"] = avg(hotspot_metrics.get("sig_agg_parallelism", []))
                        sample["cross_group_ratio"] = avg(hotspot_metrics.get("cross_group_ratio", []))
                        sample["zipf_alpha"] = avg(hotspot_metrics.get("zipf_alpha", []))
    ...
    result = ExperimentResult(
        name="实验十二：热点账户感知分组 TPBFT",
        description="验证热点负载下不同分组策略对分层 TPBFT 性能的影响",
        points=points,
        metadata={
            "groups": groups,
    ...
    csv_path = output_dir / "exp12_summary.csv"
    ...
    for algo in sig_algos:
        for alpha in zipf_alphas:
            ...
            for g in groups:
                ...
                p_random = next(
                    (p for p in points
                     ...
                     and p.params.get("alpha") == alpha
                     and p.params.get("strategy") == "random"),
                    None,
                )
                p_hotspot = next(
                    (p for p in points
                     ...
                     and p.params.get("g") == g
                     and p.params.get("alpha") == alpha
                     and p.params.get("strategy") == "hotspot"),
                    None,
                )
                if p_random or p_hotspot:
                    g_sorted.append(float(g))
                    tps_random.append(p_random.metrics.get("tps", 0.0) if p_random else 0.0)
                    tps_hotspot.append(p_hotspot.metrics.get("tps", 0.0) if p_hotspot else 0.0)
            if g_sorted and (any(v > 0 for v in tps_random) or any(v > 0 for v in tps_hotspot)):
                from analysis.svg_chart import multi_line_chart_svg
                series = []
                if any(v > 0 for v in tps_random):
                    series.append(("random", tps_random))
                if any(v > 0 for v in tps_hotspot):
                    series.append(("hotspot", tps_hotspot))
                # fallback to simple line chart if multi-line not available
                if len(series) == 1:
                    write_svg(
                        figures_dir / f"tps_vs_g_{algo}_alpha{alpha}.svg",
                        line_chart_svg(g_sorted, series[0][1], f"TPS vs g ({algo}, α={alpha})", "g", "TPS"),
                    )
                else:
                    # Try multi-line if available, else two separate
                    try:
                        svg = multi_line_chart_svg(
                            g_sorted,
                            series,
                            f"TPS vs g ({algo}, α={alpha})",
                            "g",
                            "TPS",
                        )
                        write_svg(figures_dir / f"tps_vs_g_{algo}_alpha{alpha}.svg", svg)
                    except Exception:
                        for label, values in series:
                            write_svg(
                                figures_dir / f"tps_vs_g_{algo}_alpha{alpha}_{label}.svg",
                                line_chart_svg(g_sorted, values, f"TPS vs g ({algo}, α={alpha}, {label})", "g", "TPS"),
                            )

            # Commit 耗时 vs g（按 strategy 分线）
            commit_random = []
            commit_hotspot = []
            for g in groups:
                if g <= 0 or args.nodes % g != 0:
                ...
                commit_random.append(p_random.metrics.get("commit_ms", 0.0) if p_random else 0.0)
                commit_hotspot.append(p_hotspot.metrics.get("commit_ms", 0.0) if p_hotspot else 0.0)
            if g_sorted:
                if any(v > 0 for v in commit_random):
                    write_svg(
                        figures_dir / f"commit_ms_vs_g_{algo}_alpha{alpha}_random.svg",
                        line_chart_svg(g_sorted, commit_random, f"Commit(ms) vs g ({algo}, α={alpha}, random)", "g", "Commit(ms)"),
                    )
                if any(v > 0 for v in commit_hotspot):
                    write_svg(
                        figures_dir / f"commit_ms_vs_g_{algo}_alpha{alpha}_hotspot.svg",
                        line_chart_svg(g_sorted, commit_hotspot, f"Commit(ms) vs g ({algo}, α={alpha}, hotspot)", "g", "Commit(ms)"),
                    )
            ...
            for g in groups:
                ...
                cgr_random.append(p_random.metrics.get("cross_group_ratio", 0.0) if p_random else 0.0)
                cgr_hotspot.append(p_hotspot.metrics.get("cross_group_ratio", 0.0) if p_hotspot else 0.0)
            if g_sorted:
                if any(v > 0 for v in cgr_random):
                    write_svg(
                        figures_dir / f"cross_group_ratio_vs_g_{algo}_alpha{alpha}_random.svg",
                        line_chart_svg(g_sorted, cgr_random, f"CrossGroupRatio vs g ({algo}, α={alpha}, random)", "g", "CrossGroupRatio"),
                    )
                if any(v > 0 for v in cgr_hotspot):
                    write_svg(
                        figures_dir / f"cross_group_ratio_vs_g_{algo}_alpha{alpha}_hotspot.svg",
                        line_chart_svg(g_sorted, cgr_hotspot, f"CrossGroupRatio vs g ({algo}, α={alpha}, hotspot)", "g", "CrossGroupRatio"),
                    )

        # α 敏感度曲线（仅 32 节点或当前最大节点数）
        g_fixed = max(groups) if groups else 32
        for strategy in strategies:
            alpha_valid = []
            tps_values = []
            commit_values = []
            ...
            for alpha in zipf_alphas:
                p = next(
                    (p for p in points
                     if p.params.get("algo") == algo
                     and p.params.get("g") == g_fixed
                     and p.params.get("alpha") == alpha
                     and p.params.get("strategy") == strategy),
                    None,
                )
                if p:
                    alpha_valid.append(alpha)
                    tps_values.append(p.metrics.get("tps", 0.0))
                    commit_values.append(p.metrics.get("commit_ms", 0.0))
                    cgr_values.append(p.metrics.get("cross_group_ratio", 0.0))
            if alpha_valid:
                write_svg(
                    figures_dir / f"tps_vs_alpha_{algo}_{strategy}_g{g_fixed}.svg",
                    line_chart_svg(alpha_valid, tps_values, f"TPS vs α ({algo}, {strategy}, g={g_fixed})", "α", "TPS"),
                )
                write_svg(
                    figures_dir / f"commit_vs_alpha_{algo}_{strategy}_g{g_fixed}.svg",
                    line_chart_svg(alpha_valid, commit_values, f"Commit(ms) vs α ({algo}, {strategy}, g={g_fixed})", "α", "Commit(ms)"),
                )
                write_svg(
                    figures_dir / f"cgr_vs_alpha_{algo}_{strategy}_g{g_fixed}.svg",
                    line_chart_svg(alpha_valid, cgr_values, f"CrossGroupRatio vs α ({algo}, {strategy}, g={g_fixed})", "α", "CrossGroupRatio"),
                )
    ...
    append_tps_vs_tx_by_nodes_chart(
        ...
        line_figure_name="exp12_tps_vs_tx_by_nodes.svg",
        bar_figure_name="exp12_tps_vs_tx_by_nodes_bar.svg",
        title="实验12 性能曲线（TPS）",
