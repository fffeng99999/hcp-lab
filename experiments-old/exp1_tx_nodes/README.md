...
## 如何运行
- 入口脚本：[run_exp1_tx_nodes.sh](file:///home/hcap-dev/hcap-project-experiment/hcap-lab/experiments/exp1_tx_nodes/run_exp1_tx_nodes.sh)

```bash
bash hcap-lab/experiments/exp1_tx_nodes/run_exp1_tx_nodes.sh
```

可通过环境变量覆盖默认参数：

```bash
NODES_LIST="4,8,16,32" TX_LIST="100,1000" PORT_OFFSET=1100 CHAIN_ID="hcap-exp1" \
  bash hcap-lab/experiments/exp1_tx_nodes/run_exp1_tx_nodes.sh
```

## 实验文件夹用途说明
- `run_exp1_tx_nodes.sh`：设置默认参数并转调测试脚本。
- `test_exp1_tx_nodes.sh`：构建 `hcap-loadgen`，设置实验产物目录，启动实验执行器（Python）。
...
- 节点二进制（启动脚本构建输出）：`tests/exp1_tx_nodes/bin/hcapd`
...
- `PORT_OFFSET` 用于避免端口冲突；第一个节点的常用端口：
  ...
  - 其余节点在此基础上按固定步长错开，详见启动脚本 [start_nodes.sh](file:///home/hcap-dev/hcap-project-experiment/hcap/start_nodes.sh)。
